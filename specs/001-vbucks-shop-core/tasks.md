...
- [x] T068 [P] [US4] Unit test for `services/access-gate.ts` in `__tests__/unit/services/access-gate.test.ts`: `no_username` when fortnite_username null; `friend_request_not_accepted` when status is `not_sent` or `pending`; `waiting_period` when accepted < 48 h ago (includes `hoursRemaining`); `eligible` when accepted >= 48 h ago; `unauthenticated` when profile is null (expanded scope)
- [x] T069 [P] [US4] Unit test for order placement admin notification in `__tests__/unit/api/orders.test.ts`: `POST /api/orders` success case sends admin email via `sendOrderPlacedNotificationToAdmin`
- [x] T070 [P] [US4] Unit test for `services/orders.ts` admin functions in `__tests__/unit/services/orders-admin.test.ts`: `getPendingOrders` returns all `status='pending'` rows joined with `fortnite_username`; `fulfillOrder` with `'gifted'` updates status + `resolved_at` + calls `sendOrderFulfilledNotificationToAdmin`; `fulfillOrder` with `'refunded'` calls `refund_order` RPC + calls `sendOrderRefundedNotificationToAdmin`
- [x] T071 [P] [US4] Unit test for `PATCH /api/admin/orders/[orderId]` in `__tests__/unit/api/admin-orders.test.ts`: 401 unauthenticated, 403 non-admin, 400 invalid body, 400 invalid status, 404 order not found, 409 order not pending, 200 gifted success, 200 refunded success, 500 unexpected error
...
- [x] T073 [US4] Implement `services/access-gate.ts`: export `canAccessItemShop(profile: Profile | null): AccessGateResult` — handles `unauthenticated` (null profile) plus the 4 original reasons; add `import 'server-only'` guard
- [x] T074 [US4] Update item shop page at `app/(shop)/item-shop/page.tsx`: made public (no auth required); calls `canAccessItemShop(profile | null)`; shows `<ShopBanner>` when not eligible; catalog always rendered (unauthenticated visitors can browse)
- [x] T075 [P] [US4] Update item shop detail page at `app/(shop)/item-shop/[offerId]/page.tsx`: made public; shows `<ItemShopAccessGate>` instead of `<BuyFlow>` when not eligible; no redirect (gate message shown inline)
- [x] T076 [US4] Extend `services/email.ts`: all 4 email functions implemented — vbucks purchase, order placed, order fulfilled, order refunded; errors logged and swallowed, never thrown to caller
- [x] T077 [US4] Extend `services/orders.ts` with admin functions: `getPendingOrders()` joins `skin_orders` + `profiles`; `fulfillOrder()` validates pending state, updates status, calls `refund_order` RPC for refunds, fires non-blocking admin email
- [x] T078 [US4] `PATCH /api/admin/orders/[orderId]` in `app/api/admin/orders/[orderId]/route.ts`: auth.protect → admin check → status validation → fulfillOrder → 200 ok / 404 / 409 / 500
- [x] T079 [P] [US4] Admin pending orders UI implemented as `SkinOrdersPanel` + `OrderRow` in `app/admin/orders/_components/` (component names differ from spec; behaviour matches)
- [x] T080 [P] [US4] Gift/Refund action buttons implemented inside `OrderRow` (client component) with `data-testid="gift-order-btn"` and `data-testid="refund-order-btn"`
- [x] T081 [US4] `POST /api/orders/route.ts` fires `sendOrderPlacedNotificationToAdmin` on success; fire-and-forget with error logging
- [x] T082 [US4] Admin orders page at `app/admin/orders/page.tsx` fetches and renders both purchasers panel and pending skin orders panel
- [ ] T083 [P] [US4] Order history section — deferred to profile page (not wallet); will include skin name, V-Bucks cost, status badge (pending / gifted / refunded), `resolved_at` timestamp, and `data-testid="order-status-{orderId}"`
